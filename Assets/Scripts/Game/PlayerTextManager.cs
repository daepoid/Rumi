...
public class PlayerTextManager : MonoBehaviourPunCallbacks
{
    ...
    {
        photonView.RPC("PrintCardNumberText", RpcTarget.All);
    }
    
    [PunRPC]
    void PrintCardNumberText()
    {
        //현재 플레이어들 정보 가져오기 이미 있
        playersInfo = MyGameManager.PLAYERS;
        ...
        {
            player = playersInfo.Find(x => x.NickName == PhotonNetwork.PlayerListOthers[i].NickName);
            PlayerText[2 * i + 1].text = player.cards.Length.ToString();
        }
    }
